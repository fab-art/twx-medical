...
import { emptyClassifications, emptyActDeductions, emptyActReasons } from '@/lib/rssb/config';
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const newCards = json.map((row, i) => ({
        ...
        facilityOverride: file.name.replace(/\.(xlsx|xls|csv)$/i, '').split(' - ')[0].trim(),
        explanation: '',
        classifications: emptyClassifications(),
        actDeductions: emptyActDeductions(),
        actReasons: emptyActReasons(),
        inspectionIds: [],
      }));
      ...
      <header className="rama-header flex items-center justify-between gap-4 px-4 sm:px-6 lg:px-8 py-4">
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <h1 className="text-lg font-semibold tracking-tight text-white">RSSB Medical Counter Verification System</h1>
            <p className="text-xs text-white/70">Medical claims preparation, counter verification and inspection review</p>
          </div>
        </div>
        <button
          onClick={toggle}
        ...
        <div className="max-w-2xl w-full">
          ...
          <div className="mt-6 rounded-xl border border-border bg-card p-4">
            ...
            <ol className="text-xs text-muted-foreground space-y-1.5 list-decimal list-inside">
              ...
              <li>Upload and cross-check field inspection findings</li>
              <li>Generate Counter Verification and inspection-linked reports</li>
            </ol>
          </div>
