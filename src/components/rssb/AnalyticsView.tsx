...
/** Stores a drill-down payload for the Dashboard to pick up on mount, then navigates there. */
function goToDashboardFiltered(
  setStage: (s: 'dashboard') => void,
  payload: { search?: string; classification?: string; doctor?: string; status?: 'all' | 'pending' | 'verified'; dateFrom?: string; dateTo?: string; amountMin?: number; amountMax?: number },
) {
  try {
    sessionStorage.setItem('rssb_analytics_drilldown', JSON.stringify(payload));
...
export function AnalyticsView() {
  ...
  const data = useMemo(() => {
    ...
    cards.forEach(c => {
      const p = String(helpers.mappedValue(c, 'patient_name') || '').trim();
      if (!p) return;
      if (!patientAmounts[p]) patientAmounts[p] = { count: 0, total: 0 };
      patientAmounts[p].count += 1;
      patientAmounts[p].total += helpers.originalAmount(c) || 0;
    });
    const topPatients = Object.entries(patientAmounts)
      .map(([name, v]) => ({ name, ...v }))
      .sort((a, b) => b.total - a.total)
      .slice(0, 10);
  ...
  function handleGeneratePdf() {
    ...
    const doc = buildAnalyticsPdf(data, {
      pharmacyName: counterHeader?.pharmacyName,
      fileName: fileName || undefined,
      header: counterHeader,
      sessionNotes,
    });
    doc.save(`analytics_report_${fileName || 'medical'}.pdf`);
    toast({ title: 'PDF generated', description: 'Analytics report downloaded successfully.' });
  }
  ...
  if (!cards.length) {
    return (
      <div className="rounded-xl border border-dashed border-border bg-card py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground">No data to analyze yet. Upload a medical file to see analytics.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-3">
        <p className="text-sm text-muted-foreground max-w-2xl">
          Click any KPI, chart bar, or patient to jump to the Dashboard filtered to those exact vouchers.
        </p>
        <button
          onClick={handleGeneratePdf}
          className="inline-flex items-center gap-2 text-sm rounded-lg px-4 py-2 bg-primary text-primary-foreground hover:bg-primary/90 transition-colors shrink-0"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <KPICard icon={CheckCircle2} label="Approved" value={`RWF ${data.totalApproved.toLocaleString(undefined, { maximumFractionDigits: 0 })}`} color="primary" onClick={() => goToDashboardFiltered(setStage, { status: 'verified' })} />
        ...
        <KPICard icon={CheckCircle2} label="Verification" value={`${data.verificationRate.toFixed(1)}%`} color="primary" onClick={() => goToDashboardFiltered(setStage, { status: 'verified' })} />
        <KPICard icon={ShieldAlert} label="Deduction Rate" value={`${data.deductionRate.toFixed(1)}%`} color="danger" onClick={() => goToDashboardFiltered(setStage, { classification: 'fraud' })} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          {data.dailyTrend.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={data.dailyTrend} margin={{ left: 0, right: 20 }}>
                ...
                <Area
                  type="monotone" dataKey="count" stroke={CHART_COLORS[0]} fill="url(#colorCount)" strokeWidth={2}
                  activeDot={(props: { cx?: number; cy?: number; payload?: { date?: string } }) => {
                    const { cx, cy, payload } = props;
                    return (
                      <circle
                        cx={cx} cy={cy} r={5} fill={CHART_COLORS[0]} stroke="white" strokeWidth={1.5}
                        style={{ cursor: 'pointer' }}
                        onClick={() => { if (payload?.date) goToDashboardFiltered(setStage, { dateFrom: payload.date, dateTo: payload.date }); }}
                      />
                    );
                  }}
                />
        ...
        <ChartCard title="Amount Distribution" subtitle="Voucher count and total by amount range — click a bar to view that range" icon={Activity}>
          {data.buckets.every(b => b.count === 0) ? (
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={data.buckets} margin={{ left: 0, right: 20 }}>
                ...
                <Tooltip contentStyle={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 12 }} />
                <Bar
                  dataKey="count" fill={CHART_COLORS[1]} radius={[4, 4, 0, 0]} name="Vouchers" cursor="pointer"
                  onClick={(d: { min?: number; max?: number }) => {
                    if (d?.min === undefined) return;
                    goToDashboardFiltered(setStage, { amountMin: d.min, amountMax: d.max });
                  }}
                />
              </BarChart>
            </ResponsiveContainer>
        ...
        <ChartCard title="Top Patients" subtitle="By total amount billed — click a bar to view their vouchers" icon={Users}>
          {data.topPatients.length === 0 ? (
            <EmptyChart />
          ) : (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={data.topPatients} layout="vertical" margin={{ left: 10, right: 20 }}>
                <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                <XAxis type="number" className="text-xs" tick={{ fontSize: 11 }} />
                <YAxis type="category" dataKey="name" width={130} className="text-xs" tick={{ fontSize: 10 }} />
                <Tooltip
                  contentStyle={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 12 }}
                  formatter={(v: number, key: string) => key === 'total' ? [`RWF ${v.toLocaleString()}`, 'Total billed'] : [v, 'Vouchers']}
                />
                <Bar dataKey="total" fill={CHART_COLORS[5]} radius={[0, 4, 4, 0]} name="Total billed" cursor="pointer"
                  onClick={(d: { name?: string }) => d?.name && goToDashboardFiltered(setStage, { search: d.name })} />
              </BarChart>
            </ResponsiveContainer>
          )}
...
function KPICard({ icon: Icon, label, value, color, onClick }: { icon: LucideIcon; label: string; value: string; color: 'primary' | 'danger' | 'gold'; onClick?: () => void }) {
  const colorClasses = {
  ...
  return (
    <Tag
      ...
      onClick={onClick}
      className={`rounded-xl border p-4 text-left w-full ${colorClasses} ${onClick ? 'hover:shadow-md hover:brightness-95 transition-all cursor-pointer' : ''}`}
    >
