...
import type { GraphNode, NodeType, EdgeKind } from '@/lib/rssb/network';
import { Share2, Search, ZoomIn, ZoomOut, Maximize, RotateCcw, ExternalLink, Info, Hospital, GitCompareArrows, MousePointerClick, ChevronDown, ChevronUp } from 'lucide-react';

// The layout step resolves edge endpoints from string ids to full GraphNode
// objects, so the laid-out edges have a different shape than the raw
// GraphEdge produced by buildGraph/filterGraph. Type against that resolved
// shape directly instead of the (incompatible) GraphEdge so hover/selection
// handlers type-check correctly.
type LaidEdge = ReturnType<typeof layoutGraph>['edges'][number];
...
export function NetworkGraph() {
  ...
  // Lookup so the connection preview can show real voucher details
  // (voucher number, dispensing date, amount) instead of just aggregate stats.
  ...
  const handleEdgeHover = useCallback((e: LaidEdge, idx: number, mouseEvent: React.MouseEvent) => {
    setHoveredEdge(idx);
  ...
  const openInDashboard = useCallback((node: GraphNode) => {
    // Reuse the same drill-down payload contract AnalyticsView uses so the
    // Dashboard opens already scoped to this node's vouchers. Doctors have
    // their own dedicated filter field on the Dashboard; patients and
    // facilities are matched via the free-text search box.
    try {
      const payload = node.type === 'doctor' ? { doctor: node.label } : { search: node.label };
              ...
              <g transform={`translate(${pan.x}, ${pan.y}) scale(${zoom})`}>
                ...
                {nodes.map(n => {
                  ...
                  return (
                    <g
                      key={n.id}
                      onClick={() => { setSelectedNode(n); setExpandedConnection(null); }}
        ...
        <div className="rounded-xl border border-border bg-card p-4 mt-4">
          <div className="flex items-start justify-between gap-2 mb-3">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => openInDashboard(selectedNode)}
          ...
          <div>
            ...
            <div className="max-h-64 overflow-y-auto scrollbar-thin flex flex-col gap-1">
              {edges.filter(e => e.source.id === selectedNode.id || e.target.id === selectedNode.id).map((e, i) => {
                ...
                const connKey = `${selectedNode.id}|${other.id}|${e.kind}`;
                const isExpanded = expandedConnection === connKey;
                // Sample the vouchers shared between the selected node and
                // this connection so the preview can surface real detail
                // (voucher number, dispensing date, amount) — capped at 8
                // to keep the panel readable for high-volume pairs.
                const sharedVouchers = e.voucherIds
                  .map(id => cardById.get(id))
                  .filter((c): c is NonNullable<typeof c> => !!c)
                  .slice(0, 8);
                return (
                  <div key={i} className="rounded-lg hover:bg-accent transition-colors">
                    <div className="text-left text-xs flex items-center justify-between gap-2 px-2 py-1.5">
                      <button onClick={() => { setSelectedNode(other); setExpandedConnection(null); }} className="flex items-center gap-1.5 truncate">
                      ...
                      <button
                        onClick={() => setExpandedConnection(isExpanded ? null : connKey)}
                        className="flex items-center gap-1 text-muted-foreground shrink-0 hover:text-foreground"
                        title={isExpanded ? 'Hide shared vouchers' : 'Show shared vouchers'}
                      >
                        {e.weight} shared · RWF {e.totalCost.toLocaleString()}
                        ...
                        <table className="w-full text-[11px]">
                          <thead>
                            <tr className="text-muted-foreground text-left">
                              <th className="font-medium py-1 pr-2">Voucher #</th>
                              <th className="font-medium py-1 pr-2">Dispensing date</th>
                              <th className="font-medium py-1 text-right">Amount</th>
                            </tr>
                          </thead>
                          <tbody>
                            {sharedVouchers.map(c => {
                              const dDate = helpers.dispensingDateOf(c);
                              const amt = helpers.originalAmount(c);
                              return (
                                <tr key={c.id} className="border-t border-border/60">
                                  <td className="py-1 pr-2 font-medium">{helpers.voucherOf(c) || '—'}</td>
                                  <td className="py-1 pr-2">{dDate ? dDate.toLocaleDateString() : '—'}</td>
                                  <td className="py-1 text-right tabular-nums">{amt !== null ? `RWF ${amt.toLocaleString()}` : '—'}</td>
                                </tr>
                              );
                            })}
                          </tbody>
                        </table>
                        {e.voucherIds.length > sharedVouchers.length && (
                          <p className="text-[10px] text-muted-foreground mt-1">
                            +{e.voucherIds.length - sharedVouchers.length} more voucher(s) not shown
                          </p>
                        )}
                      </div>
                    )}
