...
const InspectionView = lazy(() => import('@/components/rssb/InspectionView').then(m => ({ default: m.InspectionView })));
    ...
    return (
      <div className="min-h-screen flex flex-col">
        <header className="rama-header flex items-center justify-between gap-4 px-4 sm:px-6 lg:px-8 py-4">
          <button onClick={() => setStage('landing')} className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h1 className="text-lg font-semibold tracking-tight text-white">RSSB Medical Counter Verification</h1>
        ...
        <footer className="border-t border-border py-4 text-center text-xs text-muted-foreground">
          RSSB Medical Counter Verification System
        </footer>
      </div>
    );
  }
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          <div className="lg:hidden rama-header flex items-center justify-between gap-2 px-4 py-3 sticky top-0 z-20">
            <button onClick={() => setStage('landing')} className="flex items-center gap-2 min-w-0">
              <RssbLogo size={28} />
              <span className="truncate text-sm font-semibold text-white">RSSB Medical Verification</span>
            </button>
          ...
          <main id="main-content" tabIndex={-1} key={stage} className="px-4 sm:px-6 lg:px-8 py-6 flex-1 stage-enter">
            ...
            <Suspense fallback={
              ...
              {stage === 'inspection' && <InspectionView />}
            </Suspense>
          </main>
          ...
          <footer className="border-t border-border py-2.5 px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-3 text-xs text-muted-foreground bg-card/50">
            <span className="flex items-center gap-1.5">
              <span className="font-medium text-foreground/70">RSSB Medical CVS</span>
              <span className="text-border">·</span>
              <span>Medical claims verification</span>
              {sessionNotes && <span title="Session has notes"><NotepadText className="w-3.5 h-3.5 text-primary ml-1" /></span>}
            </span>
            <span className="flex items-center gap-2">
              {isSaving ? (
