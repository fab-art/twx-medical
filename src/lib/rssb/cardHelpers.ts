...
import { MEDICAL_ACT_DEFS } from './config';
import type { MedicalActKey } from './types';

export function actAmount(card: Card, act: MedicalActKey, mapping: Mapping): number {
  const value = mappedValue(card, act, mapping);
  const n = parseFloat(String(value).replace(/,/g, ''));
  return Number.isFinite(n) ? n : 0;
}

export function totalActBilled(card: Card, mapping: Mapping): number {
  return (MEDICAL_ACT_DEFS as readonly { key: MedicalActKey; label: string; guesses: readonly string[] }[])
    .reduce((s, d) => s + actAmount(card, d.key, mapping), 0);
}

export function totalActDeduction(card: Card): number {
  return Object.values(card.actDeductions || {}).reduce((s, n) => s + (Number(n) || 0), 0);
}

export function deductionCategories(card: Card): string[] {
  return (MEDICAL_ACT_DEFS as readonly { key: MedicalActKey; label: string }[])
    .filter(d => (Number(card.actDeductions?.[d.key]) || 0) > 0)
    .map(d => d.label);
}
