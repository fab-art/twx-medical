...
export function buildFraudReportWorkbook(
  ...
  const timesRegular: CellStyle = { font: { name: 'Times New Roman', sz: 12 } };
  ...
  // Exclude the raw voucher-number column from the source file columns — it's
  // already shown as the dedicated "Voucher No" column, so keeping both caused
  // the same voucher/paper code to appear twice in the report.
  const voucherHeader = mapping.voucher_no;
  const sourceColumns = (headers.length ? headers : (cards[0] ? Object.keys(cards[0].row) : []))
    .filter(h => h !== voucherHeader);
  ...
  aoa.forEach((row, r) => {
    ...
    row.forEach((_, ci) => {
      ...
      else if (kind === 'data') ws[addr].s = timesRegular;
...
export function buildCounterReportWorkbook(
  ...
  const aoa: unknown[][] = [
    [counterHeader.code ? `CODE/FACILITY: ${counterHeader.code}` : 'CODE/FACILITY:'],
