...
import type {
  ...
  MatchResult, Mapping, SessionMeta, Stage, InspectionFinding, InspectionMapping,
} from '@/lib/rssb/types';
import { emptyCounterHeader, emptyActDeductions, emptyActReasons, emptyInspectionMapping, SAVE_DEBOUNCE_MS } from '@/lib/rssb/config';
import {
...
interface SessionStore {
  ...
  setInspectionData: (data: { fileName: string; headers: string[]; mapping: InspectionMapping; findings: InspectionFinding[] }) => void;
  updateInspectionFinding: (id: string, patch: Partial<InspectionFinding>) => void;
...
function emptyState() {
  return {
    ...
    inspectionFileName: '',
    inspectionHeaders: [],
    inspectionMapping: emptyInspectionMapping(),
    inspectionFindings: [],
  };
}
...
export const useSessionStore = create<SessionStore>((set, get) => ({
  ...
  setInspectionData: (data) => { set({ inspectionFileName: data.fileName, inspectionHeaders: data.headers, inspectionMapping: data.mapping, inspectionFindings: data.findings, isDirty: true }); scheduleSave(get); },
  updateInspectionFinding: (id, patch) => { set(state => ({ inspectionFindings: state.inspectionFindings.map(f => f.id === id ? { ...f, ...patch } : f), isDirty: true })); scheduleSave(get); },
  ...
  loadSession: async (id) => {
    ...
    try {
      ...
      set({
        ...
        inspectionFileName: state.inspectionFileName || '',
        inspectionHeaders: state.inspectionHeaders || [],
        inspectionMapping: state.inspectionMapping || emptyInspectionMapping(),
  ...
  persist: async () => {
    ...
    try {
      ...
      const state = {
        ...
        inspectionFileName: s.inspectionFileName,
        inspectionHeaders: s.inspectionHeaders,
        inspectionMapping: s.inspectionMapping,
